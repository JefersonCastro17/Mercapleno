...
<<<<<<< HEAD
  ...
  describe('GET /api/admin/users/document-types', () => {
    it('debe listar los tipos de identificación configurados en la BD (200 OK)', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/admin/users/document-types')
        .set('Authorization', `Bearer ${adminToken}`)
        .set('x-api-key', apiKey);
